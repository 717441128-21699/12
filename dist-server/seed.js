...
exports.initSeedData = initSeedData;
...
async function initSeedData() {
    const storesCount = db_1.default.prepare('SELECT COUNT(*) as count FROM stores').get().count;
    const categoriesCount = db_1.default.prepare('SELECT COUNT(*) as count FROM categories').get().count;
    const usersCount = db_1.default.prepare('SELECT COUNT(*) as count FROM users').get().count;
    ...
    const insertStore = db_1.default.prepare('INSERT INTO stores (id, name, address, city) VALUES (?, ?, ?, ?)');
    const insertUser = db_1.default.prepare('INSERT INTO users (id, role, name, phone, password_hash, store_id, member_level, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    const insertCategory = db_1.default.prepare('INSERT INTO categories (id, name, icon, description, base_price, color) VALUES (?, ?, ?, ?, ?, ?)');
    const insertPricingRule = db_1.default.prepare('INSERT INTO pricing_rules (id, level, level_name, discount_rate, single_price, monthly_price, quarterly_price, yearly_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    const insertCourse = db_1.default.prepare('INSERT INTO courses (id, category_id, coach_id, store_id, title, date, start_time, end_time, capacity, booked_count, price, status, description, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const insertBooking = db_1.default.prepare('INSERT INTO bookings (id, member_id, course_id, status, price, discount_rate, actual_price, attendance, training_report, booked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const insertWaiting = db_1.default.prepare('INSERT INTO waiting_queue (id, course_id, member_id, position, joined_at) VALUES (?, ?, ?, ?, ?)');
    const insertRefund = db_1.default.prepare('INSERT INTO refunds (id, booking_id, member_id, total_sessions, completed_sessions, refund_ratio, paid_amount, refund_amount, status, reason, created_at, reviewed_at, reviewer_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const insertMessage = db_1.default.prepare('INSERT INTO messages (id, user_id, role, type, title, content, related_id, related_type, has_voucher, voucher_data, read, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const tx = db_1.default.transaction(() => {
        ...
        for (const s of stores)
            insertStore.run(s.id, s.name, s.address, s.city);
        const categories = [
        ...
        for (const c of categories)
            insertCategory.run(c.id, c.name, c.icon, c.description, c.basePrice, c.color);
        const pricingRules = [
        ...
        for (const p of pricingRules)
            insertPricingRule.run(p.id, p.level, p.levelName, p.discountRate, p.singlePrice, p.monthlyPrice, p.quarterlyPrice, p.yearlyPrice);
        const users = [
        ...
        for (const u of users)
            insertUser.run(u.id, u.role, u.name, u.phone, hashedPassword, u.storeId, u.memberLevel, todayISO());
        const courses = [
        ...
        for (const c of courses)
            insertCourse.run(c.id, c.categoryId, c.coachId, c.storeId, c.title, c.date, c.startTime, c.endTime, c.capacity, c.bookedCount, c.price, c.status, c.description, todayISO());
        function calcPrice(userId, basePrice) {
        ...
        for (const b of bookings)
            insertBooking.run(b.id, b.memberId, b.courseId, b.status, b.price, b.discountRate, b.actualPrice, b.attendance, b.trainingReport, b.bookedAt);
        ...
        for (const w of waitingEntries)
            insertWaiting.run(w.id, w.courseId, w.memberId, w.position, w.joinedAt);
        ...
        for (const r of refunds)
            insertRefund.run(r.id, r.bookingId, r.memberId, r.totalSessions, r.completedSessions, r.refundRatio, r.paidAmount, r.refundAmount, r.status, r.reason, r.createdAt, r.reviewedAt, r.reviewerId);
        ...
        for (const m of messages)
            insertMessage.run(m.id, m.userId, m.role, m.type, m.title, m.content, m.relatedId, m.relatedType, m.hasVoucher, m.voucherData, m.read, m.createdAt);
    });
    tx();
